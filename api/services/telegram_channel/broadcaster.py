"""
api/services/telegram_channel/broadcaster.py
----------------------------------------------
TelegramChannelBroadcaster — implements BroadcasterProtocol.
...
Reuses rate_limiter, circuit_breaker, scoring directly from the twitter
module — no code duplication of those generic components.
...
from api.services.telegram_channel.client import TelegramChannelClient, TelegramChannelClientError
from api.services.telegram_channel.templates import TelegramMessageRenderer
...
class TelegramChannelBroadcaster:
    """
    Production-grade Telegram Channel broadcasting plugin.
    ...
    def __init__(
        self,
        config: object,          # TelegramChannelConfig from settings
        ...
        self._renderer = TelegramMessageRenderer()

        # Telegram client (only if not dry_run)
        self._client: TelegramChannelClient | None = None
        if not config.dry_run:  # type: ignore[attr-defined]
            self._client = TelegramChannelClient(
                bot_token=config.bot_token,  # type: ignore[attr-defined]
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(
            self._posting_loop(), name="telegram_channel_broadcaster"
        )
        mode = "DRY-RUN" if self._config.dry_run else "LIVE"  # type: ignore[attr-defined]
        logger.info("TelegramChannelBroadcaster started (%s mode)", mode)
    ...
    async def stop(self) -> None:
        ...
        if self._task and not self._task.done():
            ...
            try:
                ...
                pass
        logger.info(
            "TelegramChannelBroadcaster stopped (queue had %d items)", self._queue.qsize()
        )
    ...
    async def handle_event(self, event: AlertDTO) -> None:
        ...
        try:
            ...
            logger.debug(
                "TelegramChannel queue: enqueued %s alert #%d (score=%.1f, depth=%d)",
                event.alert_type.value, event.alert_id, score, self._queue.qsize(),
            )
        except Exception as exc:
            logger.error("TelegramChannelBroadcaster.handle_event error: %s", exc)
    ...
    def _handle_overflow(self, new_item: ScoredAlert) -> None:
        ...
        items.sort()  # ScoredAlert.__lt__ is inverted → higher score first
        ...
        logger.warning(
            "TelegramChannel queue overflow: dropped alert #%d (score=%.1f)",
    ...
    async def _posting_loop(self) -> None:
        ...
        logger.info("TelegramChannel posting loop started")
        ...
        while self._running:
            ...
            try:
                ...
                logger.error("TelegramChannel posting loop error: %s", exc)

            # Small delay between posts (Telegram allows ~30 msg/s but we're conservative)
    ...
    async def _process_alert(self, scored: ScoredAlert) -> None:
        ...
        if not self._rate_limiter.acquire(reserve_type=reserve_type):
            logger.warning(
                "TelegramChannel rate limit hit — dropping alert #%d score=%.1f pool=%s "
        ...
        message_id = await self._post_message(content)
        await self._persist(event, content, score, message_id)

        if entity_key:
            self._cooldown.record(entity_key)

    async def _post_message(self, content: str) -> str | None:
        """Post a message. Returns message_id or None. Logs on dry_run."""
        if self._config.dry_run:  # type: ignore[attr-defined]
            logger.info("📢 [DRY-RUN] Telegram Channel:\n%s", content)
            return None

        if not self._client:
            logger.error("Telegram channel client not initialized — check credentials")
            return None

        try:
            message_id = await self._client.post_message(content)
            self._circuit.record_success()
            return message_id
        except TelegramChannelClientError as exc:
            self._circuit.record_failure(exc.status_code)
            logger.error("Telegram post failed (status=%d): %s", exc.status_code, exc)
            return None
    ...
    async def _persist(
        ...
        try:
            from api.models import TelegramChannelPost  # noqa: PLC0415
            ...
            logger.error("Failed to persist telegram_channel_post: %s", exc)
    ...
    def status(self) -> dict:
        """Full status snapshot for /telegram_channel_status command."""
        return {
            "mode": "dry-run" if self._config.dry_run else "live",  # type: ignore[attr-defined]
